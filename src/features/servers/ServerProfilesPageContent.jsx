...
import { collectBautizosServidoresYEmpleadosRows, getBautizosAttendanceTypeLabel } from '../../bautizosParty.js';
...
function ServerProfilesPageContent() {
  const {
    ...
    allParticipants,    currentEvent,    filterParticipantRows,
    formatSiNo,
    globalConfig,
    ...
    hasAdminRights,
    isBautizos,    isSiValue,    participantIsActiveInEvent,
    ...
    const serveAreaPickList = (globalConfig?.serveAreaOptions?.length ? globalConfig.serveAreaOptions : DEFAULT_SERVE_AREA_OPTIONS).filter(
      (x) => String(x || '').trim() !== ''
    );
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="min-w-0">
            <h3 className="text-lg font-bold text-slate-800 mb-1 flex items-center gap-2">
              ...
              {isBautizos ? 'Servidores y empleados' : 'Servidores - Página adicional'}
            </h3>
