...
/** Valor interno canónico (sin tilde) para guardar y comparar con isSiValue(). */
const SI = 'Si';
...
const App = () => {
    ...
    (async () => {
      ...
      if (profile.role !== 'SuperUsuario') {
        ...
        if (others > 0) {
          await signOut(auth);
          setLoginError(
            'Ya hay una sesión activa con este usuario en otra pestaña o dispositivo. Cierra esa sesión, usa «Salir» allá, o espera unos segundos e intenta de nuevo.'
          );
          return;
        }
      }
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (user.role !== 'SuperUsuario') {
        ...
        if (others > 0) {
          await signOut(auth);
          setLoginError('Ya hay una sesión activa con este usuario en otra pestaña o dispositivo. Cierra esa sesión, usa «Salir» allá, o espera unos segundos e intenta de nuevo.');
          loginInProgressRef.current = false;
          setLoginBusy(false);
          return;
        }
  ...
  const validateForm = (entry, minDep, evType) => {
    ...
    if (evType === 'Campa') {
      ...
      /* Empleado/cortesía antes que beca: evita bloqueo si beca y asistencia especial quedan inconsistentes al editar. */
      }
  ...
  const deleteSelectedLogs = async () => {
    ...
    const logEntry = {
      ...
      details: `El SuperUsuario intent? borrar ${selectedLogs.size} registro(s) de actividad. Eliminados: ${deletedCount}. Protegidos (< 30 días en depuración): ${blockedCount}.`,
      isHidden: true,
    };
            ...
            <div className="bg-white rounded-3xl p-8 shadow-2xl w-full max-w-sm animate-in zoom-in-95 duration-200 text-center">
              ...
              <p className="text-sm text-slate-500 mb-6">
                {restoreModal.type === 'single' 
                  ...
                  ? `¿Estás seguro de deshacer TODOS los cambios desde el evento "${restoreModal.log?.action}" hasta ahora? Esta acción revertir? múltiples operaciones.`
                ...
                <details className="group/camp mt-2 rounded-lg border border-slate-200/80 bg-slate-50/40">
                  ...
                  <div className="border-t border-slate-200/60 px-2.5 pb-2.5 pt-2 space-y-2 text-[10px] text-slate-600">
                    ...
                    <p className="text-[10px] text-slate-500 border-t border-slate-200/50 pt-2">
                      ...
                      {newRegCampPreview ? (
                        <> - {newRegCampPreview.concept || 'Campaña'} (sin campaña: ${newRegBaseList.toLocaleString('es-MX')}).</>
                      ) : (
            ...
            <tbody className="divide-y divide-slate-100">
              {rows.length === 0 ? (
                ...
                <tr key={`srv-${p.id}`} className="hover:bg-slate-50/60">
                  ...
                  <td className="px-3 py-2 text-slate-600">{isSiValue(p.isMarried) ? `${SI_LABEL}${p.spouseName ? ` (${p.spouseName})` : ''}` : 'No'}</td>
                  <td className="px-3 py-2 text-slate-600">{isSiValue(p.goesWithChildren) ? `${SI_LABEL}${(p.childrenCount && String(p.childrenCount).trim()) ? ` (${p.childrenCount})` : ''}` : 'No'}</td>
          ...
          <div className="bg-white rounded-3xl p-8 shadow-2xl w-full max-w-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className={`mt-5 p-4 rounded-2xl border space-y-3 ${pricingForm.type === 'fixed' ? 'border-indigo-200 bg-indigo-50/50' : 'border-amber-200 bg-amber-50/40'}`}>
              ...
              <p className={`text-[11px] ${pricingForm.type === 'fixed' ? 'text-indigo-700' : 'text-amber-700'}`}>
                Define concepto y monto a liquidar. <strong>Inicio y fin son opcionales:</strong> con ambas fechas, la campaña puede aplicarse sola cuando está vigente; sin fechas completas, solo al elegirla en el alta (pago) o al editar un registro.
              </p>
              ...
              <Settings2 className="text-indigo-600" /> Configuración de Precios
        ...
        >
          ...
          >
            ...
            <h3 className="text-xl font-black text-slate-800 mb-2">
              ...
              <Users className="text-amber-600" /> Opciones: ¿En qué área les gustaría servir?
            ...
            <div className="flex gap-3">
              <button
              <Activity className="text-orange-600" /> Categorías de alergias
