...
const buildUserAccessDoc = (user) => ({
  authUid: String(user.authUid || ''),
  authEmail: String(user.authEmail || ''),
...
const syncUserAccessDoc = async (user) => {
  if (!user?.authUid || !user?.id) return;
  await setDoc(getDocRef('app_user_access', String(user.authUid)), buildUserAccessDoc(user), { merge: true });
};
...
const normalizeIdText = (txt) =>
  String(txt || '')
    ...
    .replace(/\u00f1/gi, 'n')
    // German sharp-s maps to SS when uppercased.
    .replace(/\u00df/g, 'ss')
    .toUpperCase();
...
const App = () => {
  ...
  useEffect(() => {
    ...
    if (!pending) return;
    (async () => {
      const linked = { ...pending, authUid: fbUser.uid, authEmail: fbUser.email };
      await updateDoc(getDocRef('app_users', String(pending.id)), {
        authUid: fbUser.uid,
        authEmail: fbUser.email,
      });
      await syncUserAccessDoc(linked);
    })().catch(console.error);
  }, [fbUser, users]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      await syncUserAccessDoc(activeProfile);
      if (cancelled) return;
    ...
    if (type === 'single') {
      ...
      try {
        ...
        for (const p of backupParticipants) {
          if (!p || p.id == null) throw new Error('Backup participant without id');
          backupParticipantIds.add(String(p.id));
        }
        for (const e of backupEvents) {
          if (!e || e.id == null) throw new Error('Backup event without id');
          backupEventIds.add(String(e.id));
        }

        // Write backup docs first so a failed restore cannot leave the system empty.
  ...
  const handleAddUser = async (e) => {
    ...
    try {
      ...
      await syncUserAccessDoc(userToSave);
    } catch (err) {
      console.error(err);
      try {
  ...
  const handleUpdateUser = async (e) => {
    ...
    await syncUserAccessDoc(updatedUserAccess);
  ...
  const handleDeleteUser = async (id, username) => {
    ...
    if (userToDelete?.authUid) {
      await deleteDoc(getDocRef('app_user_access', String(userToDelete.authUid))).catch(() => {});
    }
  ...
  const submitAbono = async () => {
    ...
    const participantId = String(paymentModal.id);
    let txResult;
    try {
      txResult = await runTransaction(db, async (transaction) => {
        const ref = getDocRef('app_participants', participantId);
        ...
        const liveBaseCost = Number(getLiquidationTarget(person)) || Number(baseCost) || 0;
        const paidGrossNow = parseFloat(person.paid || 0) || 0;
        if (paidGrossNow + addedAmount > liveBaseCost) {
          const err = new Error('payment-exceeds-balance');
          err.code = 'payment-exceeds-balance';
          err.maxAmount = Math.max(liveBaseCost - paidGrossNow, 0);
          throw err;
        }
        ...
        return { person, newPaidGross, isLiquidado, newPaymentRecord, paidGrossNow };
      });
    } catch (err) {
      ...
      if (err?.code === 'participant-cancelled') {
      ...
      if (err?.code === 'payment-exceeds-balance') {
        setPaymentModal(prev => ({ ...prev, error: `El abono supera el costo total. M?ximo a abonar: $${err.maxAmount}` }));
        return;
      }
      throw err;
    }
    const { person, newPaidGross, isLiquidado, newPaymentRecord, paidGrossNow } = txResult;
    addLog(
      'Abono Financiero',
      `Registr? un abono de $${addedAmount} (${paymentMethod}${newPaymentRecord.reference ? `, Ref: ${newPaymentRecord.reference}` : ''}) para ${paymentModal.personName} en la sede ${paymentModal.loc}${paymentService ? ` (Servicio: ${paymentService})` : ''}. (Pagado: $${paidGrossNow} -> $${newPaidGross})${isLiquidado ? ' [LIQUIDADO]' : ''}`,
      null,
      null,
