...
import { app, secondaryApp, usernameToAuthEmail, loginIdentifierToAuthEmail, AUTH_EMAIL_DOMAIN, buildUsernameCandidates } from './firebaseConfig';
import {
  getAuth,
...
const App = () => {
  ...
  const handleLogin = async (e) => {
    ...
    if (!email) {
      ...
      setLoginBusy(false);
      setLoginError('Ingresa tu usuario o el correo de Firebase Authentication.');
      return;
    }
    try {
  ...
  const handleAddUser = async (e) => {
    ...
    try {
      ...
      showToast(
        err?.code === 'auth/email-already-in-use'
          ...
          : 'No se pudo crear la cuenta en Firebase Authentication.'
      );
      return;
    }
    ...
    try {
      ...
      showToast('No se pudo guardar el perfil en la base de datos. La cuenta en Authentication se ha revertido.');
      return;
    }
    await signOut(secondaryAuth).catch(() => {});
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100 bg-slate-50/80">
            ...
            <p className="text-[11px] text-slate-500 mt-0.5">
              La contraseña se registra en <span className="font-semibold text-slate-600">Firebase Authentication</span> (acceso al sistema). El rol y permisos se guardan en la base de datos de la app (Firestore), no sustituyen a Authentication.
            </p>
          </div>
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              <div>
                ...
                <div className="relative mt-1">
                  ...
                  <input type="text" className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-slate-800 outline-none focus:ring-2 focus:ring-blue-600 transition-all" placeholder="Ej. admin o el correo de Authentication" value={loginForm.username} onChange={e => setLoginForm({ ...loginForm, username: e.target.value })} />
                </div>
              </div>
              <div>
                <label className={labelClasses}>Contraseña</label>
              ...
              <p className="text-[11px] text-slate-500 leading-snug text-center">
                Contraseña: la de Firebase Authentication. Correo que usará el inicio de sesión con lo que escribiste arriba:{' '}
