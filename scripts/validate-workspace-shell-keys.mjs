/**
 * Validates workspaceShell shorthand keys exist in App scope before mergeWorkspaceShellParts.
 */
import fs from 'fs';
...
for (const line of app.slice(appStart, shellStart).split('\n')) {
  if (depth >= 1) {
...
const keys = [...new Set([...shellBlock.matchAll(/^\s+([A-Za-z_$][\w$]*)\s*,?\s*$/gm)].map((m) => m[1]))];

const missing = keys
  .filter((k) => !k.startsWith('onReload') && !bindings.has(k) && !imports.has(k))
  .sort();

console.log('workspaceShell keys:', keys.length);
console.log('missing before shell:', missing.length);
if (missing.length) {
